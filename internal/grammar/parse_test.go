...
var validCases = []struct {
	...
	{"a credential stored knowingly",
		...
		Stmt{Verb: Alter, Object: Env, Name: "e", Clauses: []Clause{
			{Kind: SetVar, Vars: []Var{{Key: "ANTHROPIC_AUTH_TOKEN", Value: "abc"}, {Key: "ANTHROPIC_MODEL", Value: "glm"}}, Plaintext: true},
			{Kind: UnsetVar, Keys: []string{"X"}}}}},
	...
	{"credential-looking keys holding settings",
		w("ALTER PLAYBOOK k SET VAR MAX_THINKING_TOKENS=8000 CLAUDE_CODE_MAX_OUTPUT_TOKENS=32000 AUTH_ENABLED=true API_KEY="),
		Stmt{Verb: Alter, Object: Playbook, Name: "k", Clauses: []Clause{{Kind: SetVar, Vars: []Var{
			{Key: "MAX_THINKING_TOKENS", Value: "8000"}, {Key: "CLAUDE_CODE_MAX_OUTPUT_TOKENS", Value: "32000"},
			{Key: "AUTH_ENABLED", Value: "true"}, {Key: "API_KEY", Value: ""}}}}}},
	{"show envs", w("SHOW ENVS"), Stmt{Verb: Show, Object: Envs}},
	{"a bare show lists the playbooks", w("show"), Stmt{Verb: Show, Object: Playbooks}},
	{"a bare show as json", w("SHOW --json"), Stmt{Verb: Show, Object: Playbooks, JSON: true}},
	...
	{"apply dry run", w("APPLY playbook.cpb --dry-run"), Stmt{Verb: Apply, File: "playbook.cpb", DryRun: true}},
	{"apply", w("apply playbook.cpb"), Stmt{Verb: Apply, File: "playbook.cpb"}},
	{"apply with drops confirmed", w("APPLY playbook.cpb --yes --dry-run"), Stmt{Verb: Apply, File: "playbook.cpb", DryRun: true, Yes: true}},
...
func TestParseArgsInvalid(t *testing.T) {
	cases := []struct {
		...
		{w("SHOW CREATE ALL --json"), "unexpected word"},
		{w("DROP ENV e --json"), "unexpected word"},
		...
		{w("ALTER ENV e SET A=1 AS"), "expected PLAINTEXT after AS"},
		{w("ALTER ENV e SET A FROM keychain:x AS PLAINTEXT"), "AS PLAINTEXT applies to literal values"},
		{w("ALTER PLAYBOOK k SET VAR GITHUB_TOKEN=ghp_x"), "use SET VAR GITHUB_TOKEN FROM '<ref>'"},
		{w("ALTER ENV e SET GITHUB_TOKEN=ghp_x"), "use SET GITHUB_TOKEN FROM '<ref>'"},
...
func TestExpect(t *testing.T) {
	cases := []struct {
		...
		{w("EXPLAIN PLAYBOOK k"), []string{"--json"}},
