...
- Name is the profile store → `".env-profiles" is the registry's env profile store, not a playbook; remove a profile with 'claude-playbook env-profile <name> delete'`. Discovery skips dot-prefixed entries, so without this guard the name would reach the orphan path below and remove every profile and the default marker in one confirmation. Matched by name and by file identity (`Lstat`, so a case variant on a case-insensitive filesystem is refused while a leftover symlink pointing at the store is still deletable, the link alone going). A real directory whose physical subtree contains the store's resolved location (the store symlinked into it) is refused as well: `"<name>" contains the registry's env profile store (<store> resolves to <path>); move the store out or remove profiles with 'claude-playbook env-profile <name> delete' first`. Both checks run before the prompt and again under the registry lock against the path actually removed, so a store created, moved, or linked while the prompt was open is still protected.
