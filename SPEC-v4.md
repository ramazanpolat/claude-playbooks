...
| `CLAUDE_CONFIG_DIR_OVERRIDE` | The config directory `run` and launcher dispatch bind, in place of the playbook's install directory. Absolute or `~`-prefixed; empty means unset; never created. Consumed -- stripped from the child's environment. Refused together with a sandboxed launch. A bare `CLAUDE_CONFIG_DIR` is still discarded. See *Environment overrides*. |
