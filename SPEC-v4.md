...
- **Retention doctrine.** `delete` and `rename` keep a launcher whose name still resolves to another playbook (`Kept command "x" (still addresses playbook "y")`). A launcher the receipt attributes to the same registry root and the playbook being removed or renamed away from was written by this tool for exactly that playbook, and is removed (`Removed command "x"`), receipt line included. Any other launcher is kept with a manual `rm <path>` hint: one made by hand, one recorded before the receipt carried attribution (v3.10.1), or one attributed to another root or playbook, because a stateless symlink may be serving something this process cannot see. Re-registering a command (`alias`, `rename`, `install`, `create`) records the current attribution, so a pre-v3.10.1 launcher becomes removable after one `claude-playbook alias <playbook> <name>`.
...
- Launcher symlinks named after the playbook or its manifest alias, claim-aware: a command name that still resolves to another playbook keeps its launcher outright (`Kept command "sre" (still addresses playbook "other")`); a launcher the receipt attributes to this registry root and this playbook is removed (`Removed command "sre"`); any other launcher is kept with a manual removal hint: `Kept command "sre" — not recorded as this playbook's own launcher, and launchers may serve other registry roots; remove it manually if unused:` followed by `rm <path>`. A kept launcher whose name no longer resolves fails loudly as stale when invoked, so retention is noisy, never silently wrong.
