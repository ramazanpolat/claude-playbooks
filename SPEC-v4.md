...
| `Sandbox` | The `[sandbox]` block in one line (`always`, `backend sbx`, `share_skills`, `secrets env`, `workdir ~/p`, `mounts ...`, `allow_net ...`, `claude 2.1.263`, comma-separated, only the keys set) when the manifest declares any; omitted otherwise |
