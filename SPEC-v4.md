...
**Install-local.** `update` carries the live block forward and ignores the source's, assembling the final manifest in the staged tree *before* the overlay so a source-shipped block is never live, even transiently; `install` drops a source-shipped block with a note. A published manifest must not be able to redirect an install's API endpoint or strip its authentication.
