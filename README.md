...
Your machine login never enters the sandbox, because `~/.claude` is exactly what stays outside. A playbook that shares it runs with a login of its own inside: `cpb` says so on the first sandboxed launch, and one `/login` there gives the playbook its own grant, stored in the playbook directory. A token from an env profile works as it does on the host.
...
`--no-sandbox` is the only override, and it is never silent. `--sbx` is a synonym for `--sandbox`; `--sandbox=BACKEND` picks the backend, of which there is one today, `sbx`.
