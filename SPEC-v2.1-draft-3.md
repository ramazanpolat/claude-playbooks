...
Discovery looks at the **immediate children** of the playbooks root only:
...
The parent's `[[children]]` table is the sole source of truth for what children exist; the filesystem is not walked to find them.
...
Children do not need their own `.playbook` file. If they have one (for example so they can also be cherry-pick installed), it is used as fallback metadata; the parent's declaration is authoritative for the tree.
...
Names are used wherever a playbook is referenced: `run`, `delete` (also `uninstall` / `unlink`), `info`, `rename`, `alias`, `dealias`, `update`.
...
Installs a playbook from a Git URL or a local directory into the playbooks root. Local sources are always copied; Git sources are shallow-cloned.

There are two install modes, selected by whether a subdirectory was specified:

- **Tree install** — no subdir. The whole source is installed. The source's root `.playbook` may declare `[[children]]` to expose child playbooks.
- **Cherry-pick install** — `--subdir <path>` or a GitHub `/tree/<branch>/<path>` URL. Only that subdirectory is installed, as a flat top-level playbook. Any `[[children]]` declarations inside it are ignored — cherry-picks are always flat.
...
# Tree install, also create child aliases
...
| Anything else | Treated as a local filesystem path; copied recursively |
...
3. Verify the target name is not taken under the playbooks root.
4. Fetch:
...
6. Write aliases:
   - Tree install: root alias by default (unless `--no-alias`). Children get aliases only with `--alias-all`, and only those children that declare an `alias` field.
   - Cherry-pick: a single alias for the installed playbook (default name = manifest alias > manifest name > install name).
7. Print a summary.
...
- Between aliases in the same install: prefix later collisions with the install name (e.g. `ap-dba` instead of `dba` when `dba` already exists).
...
- `--alias` with `--no-alias` → `--no-alias and --alias cannot be used together`
...
Symlinks an external directory into the playbooks root. Use this for **active development**: edit in your repo, see changes immediately under the playbooks root.
...
4. Check whether `<target>/.playbook` exists.
   ...
   - **Not found**: prompt the user interactively (ssh-keygen style) for `name`, `alias`, `description`. Write `.playbook` **into the target directory**. Print a clear notice that the target was modified. If stdin is not a TTY, abort with an error.
...
`link` is a separate command from `install` because it has different lifecycle semantics: deletes only affect the symlink (the target is preserved), the source is shared with whatever else uses it, and writing `.playbook` mutates the user's source directory. Keeping it separate makes the trade-off explicit.
...
```
Alias for "awesome/dba": alias ap-dba='CLAUDE_CONFIG_DIR=~/.claude-playbooks/awesome/playbooks/dba claude'
```

**One argument, no alias** — reports only; does **not** create one.
```
Playbook "awesome/dba" has no alias set.
Use 'claude-playbook alias awesome/dba <alias-name>' to create one.
```
...
Removes any aliases pointing at the named playbook. Equivalent to `claude-playbook alias <name> --remove`.
...
`uninstall` and `unlink` are accepted as command aliases for `delete` and behave identically. Use whichever reads best — there is no behavioural difference between the three:

```bash
claude-playbook delete experiment                  # prompts
claude-playbook uninstall awesome -y               # same thing, reads better after 'install'
claude-playbook unlink my-thing                    # same thing, reads better after 'link'
```

For a symlinked playbook (created with `link`), all three remove only the symlink — the target directory is preserved.
...
Delegates to `<top-level>/bin/update-playbook.sh`. Children do not have independent updaters — the top-level playbook is the unit of update.
...
**Why delegated:** playbooks come from many sources with many update strategies. A one-size-fits-all strategy is wrong for most. The playbook author writes the right logic for their own distribution.
...
Playbook name completion is wired for commands that take a name: `run`, `delete` (and its `uninstall` / `unlink` aliases), `info`, `rename`, `alias`, `dealias`, and `update`.

---

## Playbook Manifests

Every top-level playbook has a `.playbook` file at its root. The file's **presence** marks the directory as a playbook; the fields inside are metadata and (for trees) the declaration of children.

`create` writes a minimal `.playbook` automatically. `install` requires one (or `--init`). `link` writes one interactively if the target lacks it. Playbook authors can hand-write richer manifests to provide better defaults and to expose nested playbooks as children.
...
| `name` | Preferred playbook name. `install` uses it as a default suggestion; CLI `--name` overrides. The actual name is the directory under the playbooks root. Shown in `info`. |
| `alias` | Preferred alias for the top-level playbook. Used as the default when writing the alias. |
...
alias work='CLAUDE_CONFIG_DIR=~/.claude-playbooks/awesome/playbooks/dba claude --permission-mode auto --effort max'
```
